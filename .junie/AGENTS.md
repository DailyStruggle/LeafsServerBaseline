...
- **Datapacks** - you MAY run `datapacks/deploy-datapacks.ps1` without `ask_user`. It auto-discovers each `datapacks/<pack>` containing a `pack.mcmeta` and wipe-then-copies it into `<ServerBase>\<LevelName>\datapacks\<pack>` (use `-Only <pack[,pack]>` to limit which packs deploy).
